...
struct TimezoneData {
  const char* zoneName;
  const char* posixFormat;
};

const TimezoneData timezones[] PROGMEM = {
  { "Africa/Abidjan", "GMT0" },
  { "Africa/Accra", "GMT0" },
...
String TZDatabase::getPosix(String iana) {
  size_t count = sizeof(timezones) / sizeof(timezones[0]);
  for (size_t i = 0; i < count; i++) {
    if (strcmp_P(iana.c_str(), timezones[i].zoneName) == 0) {
      return FPSTR(timezones[i].posixFormat);
    }
  }
  return String("UTC0");
}